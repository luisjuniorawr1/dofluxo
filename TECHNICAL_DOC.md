...
Força clientes com o app aberto a recarregar quando uma nova versão é publicada.
...
| `app_version_service_stub.dart` | No-op (Android/desktop) |
| `lib/core/update/app_update_gate.dart` | `MaterialApp.builder` → overlay bloqueante sobre todas as rotas |
...
Cada projeto persiste `status` (coluna) e `ordem` (posição comparável). O campo
antigo `boardOrder` é aceito somente como fallback de leitura; no primeiro
snapshot, colunas com itens sem `ordem` são renumeradas em batch com passo 1024.

Fluxo de move:

1. O drop envia os IDs visíveis anterior/próximo, não uma ordem calculada pela UI.
2. `DashboardPage` resolve essas âncoras contra o board **completo**, antes dos
   filtros Job/Planejamento, e obtém os vizinhos reais.
3. `ProjectService.moveProject` relê card e vizinhos em transação.
4. `status`, `planningStatus` (somente planejamento) e `ordem` são gravados
   atomicamente.
5. Se o espaço entre ordens estiver esgotado ou houver item não migrado, a
   coluna destino é renumerada dentro da mesma transação.

O card é reposicionado otimisticamente no momento do drop. A transação normal
relê somente o card e até dois vizinhos; a leitura da coluna inteira fica
reservada ao rebalanceamento raro. Se o save falhar, o estado otimista é
removido e o stream restaura a posição persistida.

Há lock local por `projectId`: um segundo drop do mesmo card é ignorado enquanto
o primeiro save está em andamento. Falhas mostram SnackBar e o stream do
Firestore restaura o estado real. O início do drag também suprime o tap por
600 ms, evitando abrir o detalhe ao soltar.
