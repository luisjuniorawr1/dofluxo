...
class AccountPage extends StatelessWidget {
  ...
  List<Membership> _sortedMemberships(AgencyContext agencyContext) {
    ...
    memberships.sort((a, b) {
      ...
      return a.displayAgencyName.toLowerCase().compareTo(
        b.displayAgencyName.toLowerCase(),
      );
    });
  ...
  Future<void> _selectAgency(
    BuildContext context,
    Membership membership,
  ) async {
    final agencyContext = context.read<AgencyContext>();
    if (agencyContext.activeAgencyId == membership.agencyId ||
        agencyContext.isLoading) {
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Agência ativa: ${membership.displayAgencyName}'),
        ),
      );
    } catch (e) {
      if (!context.mounted) return;
