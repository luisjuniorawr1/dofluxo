...
class _ProjectDetailPageState extends State<ProjectDetailPage> {
  ...
  void _syncFromData(Map<String, dynamic> data) {
    ...
    _planningStatus = PlanningStatus.fromFirestore(data['planningStatus'] as String?);

    final format = (data['format'] as String?)?.trim() ?? PlanningFormat.options.first;
  ...
  Future<void> _updateStage(DashboardStageId stage) async {
    ...
    try {
      ...
      await context.read<ProjectService>().updateProject(widget.projectId, {
        ...
        if (_isPlanejamento && planningStatus != null) 'planningStatus': planningStatus,
      });
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  List<Widget> _buildJobFields(BuildContext context, double? progress) {
    return [
      ...
      const SizedBox(height: 24),
      Text('Atividades de produção (até 5)', style: ThemeUtils.sectionTitle(context)),
          ...
          child: Row(
            ...
            children: [
              Checkbox(
                ...
                onChanged: task.label.trim().isEmpty ? null : (value) => _toggleTask(index, value),
              ),
              Expanded(
                child: TextField(
