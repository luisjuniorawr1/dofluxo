...
class AgencyContext extends ChangeNotifier {
  ...
  Color get activePrimaryColor => _activeAgency?.primaryColor ?? Agency.defaultPrimaryColor;
  ...
  Future<void> initialize(User user) async {
    ...
    try {
      ...
      if (_isPermissionDenied(e)) {
        _errorMessage = 'Permissão negada no Firestore durante o bootstrap.\n\n'
            'Republicar firestore.rules no Console (users, agencies, memberships).\n\n'
            'Detalhe: $e';
      } else if (_isFirestoreIndexError(e)) {
        _errorMessage = 'Índice Firestore pendente.\n\n'
  ...
  Future<AgencyInviteCode> redeemInviteCode(String rawCode) async {
    ...
    try {
      final result = await inviteCodeService.redeem(rawCode: rawCode, user: user);
  ...
  Future<void> selectAgency(String agencyId) async {
    ...
    try {
      final membership = _memberships.firstWhere(
        ...
        orElse: () => throw StateError('Membership não encontrada para a agência.'),
      );
  ...
  Future<void> _resolveActiveAgency(String userId, String? savedAgencyId) async {
    ...
    final savedIsValid = savedAgencyId != null &&
        _memberships.any((m) => m.agencyId == savedAgencyId && m.isActive);

    if (savedIsValid) {
      final membership = _memberships.firstWhere((m) => m.agencyId == savedAgencyId);
      final agency = await _agencyService.getById(
        savedAgencyId,
        preferServer: true,
      );
  ...
  Future<void> _activateAgency(Agency agency, Membership membership, String userId) async {
  ...
  Future<Membership> _ensureOwnerMembershipDoc({
    ...
    final expectedId = Membership.composeId(agencyId: agency.id, userId: userId);
