...
class InviteCodeService {
  ...
  /// Resgata convite: 1) cria membership 2) marca código usado 3) índice da agência.
  /// Se a membership for criada, o resgate é considerado sucesso mesmo se (2)/(3) falharem.
  Future<InviteCodeRedeemResult> redeem({
    ...
    try {
      ...
      final invite = AgencyInviteCode.fromFirestore(
        inviteSnap.id,
        inviteSnap.data()!,
      );
      ...
      final membershipRef =
          _db.collection('memberships').doc(resolvedMembershipId);
      ...
      // Passo crítico: criar/reativar membership enquanto o convite ainda está active.
      ...
      // Passos secundários — não bloqueiam a entrada se falharem.
      try {
        ...
        debugPrint('DOFLUXO: convite não marcado como used ($e).');
      }

      try {
        await _db.collection('agencies').doc(invite.agencyId).set(
          {
        ...
        debugPrint('DOFLUXO: activeMemberIds não atualizado ($e).');
      }

      unawaited(_unregisterActiveInviteCode(invite.agencyId, code));

      return InviteCodeRedeemResult(invite: invite, membership: membership);
    } on InviteCodeNotFoundException {
      rethrow;
    } on InviteCodeExpiredException {
      rethrow;
    } on InviteCodeAlreadyUsedException {
      rethrow;
    } on InviteCodeRevokedException {
      rethrow;
    } on InviteCodeInvalidRoleException {
      rethrow;
    } on AlreadyAgencyMemberException {
      rethrow;
    } on FirebaseException catch (e) {
      if (e.code == 'permission-denied') {
        throw StateError(
          'Permissão negada ao entrar com o convite.\n\n'
          'As regras do Firestore precisam estar publicadas.\n'
          'No Firebase CLI Win, rode:\n\n'
          'cd C:\\Users\\Junior\\developer\\dofluxo\n'
          'firebase deploy --only firestore:rules\n\n'
          'Projeto: dofluxo-organizer\n\n'
          'Depois gere um código novo e tente de novo.',
        );
      }
      rethrow;
    }
  ...
  Future<void> _ensureCreatorCanManageAgency({
    ...
    final membershipId = Membership.composeId(
      agencyId: agencyId,
      userId: userId,
    );
  ...
  Future<void> _unregisterActiveInviteCodes(
    ...
    try {
      ...
      // Novo membro normalmente não pode limpar activeInviteCodes — ok.
      debugPrint('DOFLUXO: activeInviteCodes não limpo ($e).');
    }
  }
