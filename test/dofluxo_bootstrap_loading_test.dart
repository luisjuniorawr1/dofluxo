...
void main() {
  testWidgets('DofluxoBootstrapLoading exibe ícone e texto lado a lado', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: DofluxoBootstrapLoading(),
      ),
    );
  ...
  testWidgets('DofluxoBootstrapLoading avança para a próxima mensagem', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: DofluxoBootstrapLoading(),
      ),
  ...
  test('BootstrapLoadingMessages contém exatamente 6 frases', () {
    ...
    expect(BootstrapLoadingMessages.messages.first.text, 'Reunindo a equipe...');
    expect(
